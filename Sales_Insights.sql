...
SELECT COUNTRY, PRODUCTLINE, YEAR, REVENUE FROM (
	SELECT COUNTRY, PRODUCTLINE, YEAR, REVENUE, DENSE_RANK() OVER(PARTITION BY COUNTRY, YEAR ORDER BY REVENUE DESC) AS ordering
    FROM ( 
		SELECT DISTINCT
			COUNTRY, PRODUCTLINE, YEAR(ORDERDATE) AS YEAR, SUM(QUANTITYORDERED*PRICEEACH) AS REVENUE
		FROM
			SALES
		WHERE STATUS = "Shipped"
		GROUP BY 1 , 2 , 3
) a) a WHERE ordering = 1
ORDER BY COUNTRY, PRODUCTLINE, YEAR
;
...
FROM (
	SELECT DISTINCT
		COUNTRY, CITY, SUM(QUANTITYORDERED*PRICEEACH) AS REVENUE
	FROM
		SALES
	WHERE STATUS = "Shipped"
...
SELECT 
    CONCAT(p.productcode, ',  ', s.productcode) AS products, COUNT(*) AS times_sold
FROM
    prods p
        JOIN
...
SELECT R.CUSTOMERNAME AS VALUABLE_CUSTOMERS FROM RECENCY R 
